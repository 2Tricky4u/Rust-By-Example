...
pub (crate) fn fors() {
    ...
    // mutably borrows each elem of the collection
...
pub (crate) fn iflet() {
    ...
    // That is why comparing FooEx::Bar == a fails below.
    enum FooEx {Bar}

    let a = FooEx::Bar;

    // Variable a matches FooEx::Bar
    if let FooEx::Bar = a {
